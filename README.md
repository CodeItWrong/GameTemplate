...
![alt text](https://github.com/streamitwrong/GameTemplate/blob/main/Game%20Template.png?raw=true)
...
*	Loop_Display ()
	...
	*	Put these commands in the Display Loop: Print, Sync, Camera, or Sprite Commands
*	Update_Main ()
	*	Everything that pertains to the template that needs to be updated
*	Update_Display ()
	*	Everything that pertains to the template that needs to be updated or outputted

Animation.dba
*	Setup_Animation ()
	*	Extracts the animation and writes all animations exported to an animation list
*	CharacterShop_Extract ()
	*	Extracts the Character Shop animation and No Anim object file
*	ApocalypticPack_Extract ()
	*	Extracts the Apocalyptic Pack animation and No Anim object file
*	ModelPack10_Extract ()
	*	Extracts the Model Pack 10 animation and No Anim object file
*	CharacterShop_WriteList ()
	*	Exports the full animation file list in a text file
*	ApocalypticPack_WriteList ()
	*	Exports the full animation file list in a text file
*	ModelPack10_WriteList ()
	*	Exports the full animation file list in a text file
*	CharacterShop_NoAnimAll ()
	*	Exports a copy of the object with no animations
*	ApocalypticPack_NoAnimAll ()
	*	Exports a copy of the object with no animations
*	ModelPack10_NoAnimAll ()
	*	Exports a copy of the object with no animations
*	Extract_FPSCreatorBase ()
	*	Extracts the base animation of the FPS Creator animation list. The format is the same so I can extract the base and then the extra animations.
*	Extract_CharacterShopBase ()
	*	Extracts the base animation of the Character Shop animation list. The format is the same so I can extract the base and then the extra animations.

Dynamic Resources.dba
*	Coroutine_GarbageCollection ()
	*	This coroutine checks every index for empty indexes and adds them to the delete resource arrays
...
*	Update_TimedEvents (EventIndex, StartTime#, Units$, ResetFlag)
	*	When the countdown timer reaches 0 it will call the event and restart if the ResetFlag is set to 1
	*	The EventIndex is the Index of an Event() to call
	*	The Units$ are: “Hours”, “Minutes”, “Seconds”, “MiliSeconds”
	*	StartTime#  is the time will count down from
	*	ResetFlag if set to 1 then the countdown timer will restart
*	Reset_Timer (Index)
	*	It will restart the timer back to 0
*	GetHours (Index)
